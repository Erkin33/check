export default function ProductItems() {
  return (
    <div className="w-full xs:mt-[5%] lg:h-[500px] h-[500px] md:h-[300px] flex xs:flex-col  justify-between items-center">
      ...
      {/* Правая половина */}
      <div className="w-[49%] xs:w-full xs:h-[50%] h-[80%] md:h-[100%] bg-white flex justify-between flex-col items-center ">


        <small className="text-[#CE8B39] h-[5%] xs:text-[4vw] xs:w-full xs:text-center text-[1.5rem] w-full ">
        Glamorous Life
        </small>
        <h2 className="text-[#0E343D] xs:text-[5.7vw] xs:w-full xs:text-center lg:text-[2.4rem] h-[30%] lg:text-[2.8rem] text-[2.8rem] w-full ">
        Redefining Elegance with
Unique Charms
        </h2>
        <p className="text-[#000000] xs:w-full xs:text-center xs:text-[0.8rem] h-[25%] lg:text-[1rem] text-[0.7rem] w-full z-[100]">
        ...
        <div className="w-[100%] h-[15%]  xs:justify-center flex justify-start items-center">
          <a href="#" id="next" className="xs:text-[3vw] w-[30%] text-[#ffffff] text-[3vh] lg:text-[2.5vh] font-['Arial'] h-full bg-[#0E343D] flex justify-center items-center">
          know more<img src="/KnowMore.svg" alt=""  className="w-[13%] h-[40%] ml-[5%] rounded-[100%]  bg-[#D8D8D8]"/>
          </a>
          
        </div>

        <img src="/Shorts/blue.jpg" alt=""  className="absolute xs:w-[25%]  xs:h-[20%] right-[-10%] right-[50%]  w-[15%] h-[30%] mt-[18%] rounded-t-[15%] mr-[12%]"/>
      </div>
    </div>
  );
}
