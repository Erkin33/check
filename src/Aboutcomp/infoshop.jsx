...
export default function ShippingAbout() {
    return (
        <div className="w-full mt-[5%] xs:h-auto flex justify-between items-center xs:flex-col xs:mb-[5%]">
            {shop.map((shopItem, index) => (
                <div
                    key={index}
                    id={shopItem.id}
                    className="w-[25%] border-r-[1px] border-black h-[50%] flex flex-row xs:flex-col justify-around items-center last:border-r-0 xs:w-[90%] xs:border-none xs:justify-start xs:gap-4"
                >
                    {/* Иконка */}
                    <img
                        src={shopItem.src}
                        alt=""
                        className="w-[20%] h-[40%] xs:w-[15%] xs:h-auto"
                    />
                    {/* Текстовый блок */}
                    <div className="w-[50%] xs:w-full flex flex-col justify-center text-center h-[60px]">
                        <h1 className="text-[1vw] xs:text-[4.5vw] font-semibold">
                            {shopItem.mainInfo}
                        </h1>
                        <p className="text-[0.8vw] xs:text-[3.5vw] text-gray-600">
                            {shopItem.info}
                        </p>
                    </div>
                </div>
